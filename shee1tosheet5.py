...
#新建array进行省份分析
output_data = np.zeros((3, 15), dtype = float)
prov = ["江苏", "浙江", "广东"]
output_data[0, 0] = 65
output_data[1, 0] = 87
output_data[2, 0] = 95
#数据分析
for sheet2_row_num in range(0, row_num_total):
    data = sheet2.loc[5 + sheet2_row_num].values
    #print(data)
    this_prov_index = prov.index(data[0]) #找到省份对应的索引值
    #output_data[this_prov_index, 0] += 1 #存量渠道统计
    if data[10] > 0: #当月激活渠道数
        output_data[this_prov_index, 1] += 1
    if data[9] > 0: #上月激活渠道数
        output_data[this_prov_index, 2] += 1
    output_data[this_prov_index, 11 + ord(data[30]) - ord('A')] += 1 #统计渠道结构
    output_data[this_prov_index, 9] += data[23] #当月报单

for sheet1_row_num in range(1, sheet1_dnum):
    data = sheet1.loc[sheet1_row_num].values
    this_prov_index = prov.index(data[8]) #找到省份对应的索引值
    if data[1] and not pd.isna(data[2]) and data[4] == this_month: #当月起租数
        output_data[this_prov_index, 6] += 1
    if data[1] and not pd.isna(data[2]) and data[4] == this_month - 1: #上月起租数
        output_data[this_prov_index, 8] += 1

for this_prov_index in range(len(prov)):
    
    if output_data[this_prov_index, 0] != 0:
        output_data[this_prov_index, 4] = output_data[this_prov_index, 1] / output_data[this_prov_index, 0] #本月激活率
        output_data[this_prov_index, 5] = output_data[this_prov_index, 2] / output_data[this_prov_index, 0] #上月激活率

    output_data[this_prov_index, 3] = output_data[this_prov_index, 4] - output_data[this_prov_index, 5] #激活渠道数较上月
    
    if output_data[this_prov_index, 2] != 0:
        output_data[this_prov_index, 7] = output_data[this_prov_index, 6] / output_data[this_prov_index, 1] #单店平均起租

    if output_data[this_prov_index, 3] != 0:
        output_data[this_prov_index, 8] = output_data[this_prov_index, 8] / output_data[this_prov_index, 2] #上月单店平均起租

    if output_data[this_prov_index, 9] != 0:
        output_data[this_prov_index, 10] = output_data[this_prov_index, 6] / output_data[this_prov_index, 9] #转化率


sheet5 = pd.DataFrame(output_data, index = prov, columns = None)
...
sheet5.to_excel(writer, sheet_name = '渠道统计内容')
writer.save()
writer.close()
